...
To evaluate classifiers, we use the $F_1$ score (\ref{eq:f1}) that combines recall (\ref{eq:recall}) and precision (\ref{eq:precision}) in the following way: \cite{TAN2006290}
...
Of these, the SVM model got the best accuracy of 26.89\%.
...
\begin{table*}[h]
    ...
    SVM             & 0.2466          & 0.2616       & 0.2333          & 0.2389       \\ \hline
    \end{tabularx}
...
and then use the model to predict the unlabeled data.
...
The results of all models that are mentioned in this report are shown in the table \ref{tab:all_results} (accuracy) ,table \ref{tab:f1_results} ($F_1$ score),
and the line chart (Accuracy Comparison).
...
I discover that the probability of the predicted labels do not have a high confidence (most of them are below 0.4).
